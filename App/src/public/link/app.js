document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Fetch Link Request
  try {
    const res = await fetch('/agent/link/fetch/' + transaction.id);
  ...
  form1.open_file.addEventListener('click', async (event) => {
  ...
  form1.select_file.addEventListener('click', async (event) => {
    event.preventDefault();
    const fileHandle = await FileMaster.selectFile();
